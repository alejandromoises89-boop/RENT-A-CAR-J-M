...
# --- 4. PORTAL JM ---
        ...
        for car in flota_ordenada:
            ...
            with st.container():
                ...
                with col2:
                    st.write(f"### {car['n']}")
                    if libre:
                        st.markdown(f'<p class="price-tag">Precio: {car["p"]} Reales / día</p>', unsafe_allow_html=True)
                        st.write(f"Total por {dias} días: **{total_reserva} Reales**")
                        if st.button(f"Confirmar Reserva: {car['n']}", key=car['n']):
                            ...
                            msg = f"Hola JM, soy {st.session_state.user_name}. Reservé el {car['n']} del {f_ini} al {f_fin}. Pago: {total_reserva} Reales."
                            st.markdown(f'<a href="https://wa.me/595991681191?text={urllib.parse.quote(msg)}" class="btn-wa-confirm">📲 NOTIFICAR PAGO POR WHATSAPP</a>', unsafe_allow_html=True)
                    else:
                        st.error("🔴 VEHÍCULO NO DISPONIBLE EN ESTAS FECHAS")
                st.markdown('</div>', unsafe_allow_html=True)

    with tabs[3]:
        st.subheader("Reseñas de la Comunidad")
        with st.expander("✍️ Dejar un comentario"):
        ...
        for _, row in df_f.iterrows():
            st.markdown(f'<div class="review-card"><strong style="color:#D4AF37;">{row["cliente"]}</strong> <small>({row["fecha"]})</small><br><span style="color:#FFD700;">{"★" * row["estrellas"]}</span><br>{row["comentario"]}</div>', unsafe_allow_html=True)
        conn.close()
    ...
    with tabs[4]:
        ...
        if pin == "2026":
            ...
            if not df_m.empty:
                st.metric("Utilidad Bruta", f"{df_m['monto_ingreso'].sum() - df_m['monto_egreso'].sum()} Reales")
                st.dataframe(df_m)
            conn.close()
